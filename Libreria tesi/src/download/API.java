 ...
 */
	public static void getActivityFeed (String user) {
		try {
			metafeedUrl = new URL("http://gdata.youtube.com/feeds/api/events?author=" + user);
			Contatore.incApi();
    ...
    public static void checkFlood (String tabella, String user) {
		OutputTxt.writeLog("Rete floodata dalle API.");    // DA RIFAREEEEE
		OutputTxt.writeLog("Richieste API: " + Contatore.getApi());
		OutputTxt.writeLog("Richieste URL: " + Contatore.getUrl());
		Contatore.setApi(0);
		Contatore.setUrl(0);
		System.out.println("Rete floodata dalle API.");
		try {
			//REINSERIRE CONTATTO NEL DB
			Thread.currentThread();
			Thread.sleep(300000);	 // Pausa di circa 5 minuti
		}
