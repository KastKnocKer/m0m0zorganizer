...
public class urlReader  {
    ...
    public static boolean userReader (String lista, String user, int n) {
    	...
    	try {
    	metafeedUrl = new URL("http://www.youtube.com/profile?user=" + user + "&view=" + lista + "&start=" + n);
    ...
    public static boolean friendCount (String user) {
    	...
    	try {
    	metafeedUrl = new URL("http://www.youtube.com/profile?user=" + user + "&view=friends");
		in = new BufferedReader(	new InputStreamReader(metafeedUrl.openStream()));
		String inputLine;
		while ((inputLine = in.readLine()) != null) {
			if (inputLine.contains("Siamo spiacenti per l'interruzione")) {
				...
				OutputTxt.writeLog(controllo);    // DA RIFAREEEEE
		    ...
		    if (inputLine.contains("Non è disponibile")) {
		    	metafeedUrl = new URL("http://gdata.youtube.com/feeds/api/users/" + user + "/contacts");
		    	in.close();
				try {
					in = new BufferedReader(new InputStreamReader(metafeedUrl.openStream()));
					while ((inputLine = in.readLine()) != null) {
    ...
    public static void favoritesReader (String user) {
    	favoritesReader(user , 1 , 0 , 0);
    }
    
    public static void favoritesReader (String user, int count, int totalRequest, int n) {
      	...
      	try {
			...
			while ((inputLine = in.readLine()) != null) {
			    ...
			    while (inputLine.contains("<entry><id>")) {				    	
			    	...
			    	count++;
			    	System.out.println(count -1 + ": Video Id = " + id);
			    	System.out.println("Pubblicato: " + published);
			    }
			}
			System.out.println("\n pre tot " + tot);
			if ((count-1) != (50 * n) && ((tot - 50 * n) > 50))
				tot = tot + (50 * n - count - 1 );			
			if ((count != 1 && count < tot))   {// Check se è divisibile per 50 
				System.out.println("\ntot " + tot);
				System.out.println("count " + (count-1) + "\n");
				favoritesReader (user, count, tot, n);
      	}
		}catch (IOException e) {
