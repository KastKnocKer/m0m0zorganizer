...
public class urlReader  {
	...
	public static void userReader (String tabella, String user) {
		...
		userReader (tabella, user, 0, 0, userTemp);
		userTemp.removeAllElements();
	}
	
    public static void userReader (String tabella, String user, int count, int effettivi, Vector<String> userTemp) {
		...
		while ((inputLine = in.readLine()) != null) {
			...
			else if (inputLine.contains("non ha") && count == 0) {
				in.close();
		    ...
		    if (inputLine.equals("</html>") && count != 0 && count < tot) {
		    		in.close();
		    		userReader(tabella, user, count, effettivi, userTemp);
		    		return;
		    }
		}
		in.close();
