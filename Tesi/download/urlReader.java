      	...
      	try {
			...
			while ((inputLine = in.readLine()) != null) {
				...
				while(inputLine.contains("<entry gd")) {
			    	...
			    	updated = inputLine.substring(8 , inputLine.indexOf("</updated>"));
			  ...
			  //  	OutputTxt.writeLog("Inserimento nella tabella video della tupla: " 
			  //  			+ userTemp + " - "+ id + " - " + published );
			  //  	System.out.println(count-1 + ": Inserimento nella tabella video della tupla: " 
			  //  			+ userTemp + " - "+ id + " - " + updated );
			  //  	DatabaseMySql.insert("utenti", "video" , userTemp, id, published);
			    	if (count == 951) {
			  //  		OutputTxt.writeLog("Cap dei video raggiunto per l'user: " + userTemp);
			  //  		System.out.println("Cap dei video raggiunto per l'user: " + userTemp);
			    		return;
			    	}
			    }
			}
            	...
            	in = new BufferedReader(new InputStreamReader(metafeedUrl.openStream()));
          		...
          		while ((inputLine = in.readLine()) != null) {
        			if (inputLine.contains("Siamo spiacenti per l'interruzione"))
        		    	...
        		    	tmp++;
        		    	System.out.println(tmp +": Inserimento del popular: " + inputLine);
        		    	//OutputTxt.writeLog("Inserimento nella tabella " + tabella + " della tupla: " 
        		    	//		+ user + " - " + inputLine);
        		    	//DatabaseMySql.insert("utenti", tabella , user, inputLine);	
        		    }  
